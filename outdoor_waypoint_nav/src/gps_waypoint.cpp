...
int countWaypointsInFile(std::string path_local)
{
    ...
    ROS_INFO("Absolute path parameter server: %s", path_abs.c_str());
    // path_abs = ros::package::getPath("outdoor_waypoint_nav") + "/waypoint_files/points_sim.txt";
    // ROS_INFO("Absolute path hardcoded: %s", path_abs.c_str());
    std::ifstream fileCount(path_abs.c_str());
    if(fileCount.is_open())
    {
        double lati = 0;
        // while(!fileCount.eof() && ros::ok())
        while(!fileCount.eof())
        {
            fileCount >> lati;
            ++count;
        }
        ROS_INFO("Numb of numb: %i", count);
        // count = count - 1;
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO("Connected to the move_base server");

    //Count number of waypoints
    ros::param::get("/outdoor_waypoint_nav/coordinates_file", path_local);
    ROS_INFO("Path from parameter server: %s", path_local.c_str());
    // path_local = "/waypoint_files/points_sim.txt";
    // ROS_INFO("Path hardcoded: %s", path_local.c_str());
    ...
    for(iter = waypointVect.begin(); iter < waypointVect.end(); iter++)
    {
        ...
        {
            ROS_INFO("Wolverine has reached its goal!");
            //switch to next waypoint and repeat
        }
        else
        {
            ROS_ERROR("Wolverine was unable to reach its goal. GPS Waypoint unreachable.");
    ...
    ROS_INFO("Wolverine has reached all of its goals!!!\n");
