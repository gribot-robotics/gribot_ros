...
def launch_subscribers():
    ...
    rospy.Subscriber("/key",String, key_CB )
...
def print_instructions():
    ...
    print ""
    print "Press %s to start waypoint collection" % collect_key_num
    print "Press %s to start waypoint following" % send_key_num
...
def check_buttons():
    ...
    # Collect GPS waypoint

    key_local = key

    if key_local != "None" and key_local:
        # rospy.loginfo( key_local )
        key_local = chr(int(key_local))
        if key_local == collect_key_num:
            # while key == collect_key_num:    # Wait for button to be released
            #     pass
            rospy.loginfo( "Collect key" )
            # rospy.loginfo( "collect_button_num" )
        ...
        elif key_local == send_key_num:
            # while key == send_key_num:    # Wait for button to be released
            #     pass
            rospy.loginfo( "Send key" )
            # rospy.loginfo( "send_button_num" )
        ...
        #         pass
        #     rospy.loginfo( "calibrate_button_num" )
    ...
    elif (collect_complete or send_complete):
